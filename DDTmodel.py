...
parser.add_argument('--type', type=str, default="jiezhi/", help='the category of images')

opt = parser.parse_args()
print(opt)

in_dir = opt.data_dir + opt.type
out_dir = opt.save_dir + opt.type
...
pca = PCA()
# (num_image,14,14)
projected_descriptors = pca(descriptors)
# 将所有小于0的值用0代替
projected_descriptors = torch.clamp(projected_descriptors, min=0)
# 归一化为[0,1]
# maxv : (n,1,1)
max_mat = projected_descriptors.view(num_image, -1).max(dim=1)[0].unsqueeze(1).unsqueeze(2)
...
for i, name in enumerate(image_files):
    ...
    ret, binary_image = cv2.threshold(gray_image, 128, 255, cv2.THRESH_BINARY_INV)
